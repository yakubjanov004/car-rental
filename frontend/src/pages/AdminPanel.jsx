...
const AdminPanel = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      setLoading(true);
      try {
        const [bookingsRes, carsRes, messagesRes, statsRes, usersRes, txRes] = await Promise.all([
          apiClient.get("/bookings/all/"),
          apiClient.get("/cars/"),
          apiClient.get("/contact/list/"),
          apiClient.get("/admin-panel/stats/"),
          apiClient.get("/users/admin/users/"),
          apiClient.get("/payments/transactions/") // In a real app, this would be an admin endpoint
        ]);
        ...
        <nav className="flex-1 space-y-1.5">
          {tabs.map(tab => (
            <button
              ...
              className={`w-full flex items-center gap-3 px-5 py-3.5 rounded-2xl transition-all duration-500 group relative overflow-hidden ${
                activeTab === tab.id 
                  ? "text-white" 
            ...
            >
              ...
              {tab.id === 'verifikatsiya' && users.filter(u => u.verification_status === 'pending').length > 0 && (
                <span className="ml-auto w-5 h-5 bg-orange-500 rounded-full flex items-center justify-center text-[9px] font-black shadow-lg shadow-orange-500/20">{users.filter(u => u.verification_status === 'pending').length}</span>
              )}
            </button>
          ))}
        </nav>
                 ...
                 <div key="verifikatsiya" className="space-y-8">
                   <div className="flex items-center justify-between mb-2">
                      ...
                      <div className="px-4 py-2 bg-white/5 rounded-2xl border border-white/5 text-[10px] font-black text-white/40 uppercase tracking-widest leading-none">
                         {users.filter(u => u.verification_status === 'pending').length} ariza kutilmoqda
                      </div>
                   </div>
                   ...
                   {users.filter(u => u.verification_status === 'pending').length > 0 ? (
                     <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                       {users.filter(u => u.verification_status === 'pending').map(u => (
                         <div key={u.id} className="bg-[#111]/60 backdrop-blur-3xl p-10 rounded-[56px] border border-white/5 space-y-10 group hover:border-primary/20 transition-all duration-700 relative overflow-hidden shadow-2xl">
                            ...
                            <div className="flex justify-between items-start">
                               <div className="flex items-center gap-5">
                                  ...
                                  <div className="overflow-hidden">
                                     <h4 className="text-xl font-black text-white uppercase italic truncate tracking-tighter mb-1">{u.first_name} {u.last_name}</h4>
                                     <p className="text-[10px] text-white/30 uppercase font-black tracking-widest">REG ID #{u.id.toString().padStart(6, '0')}</p>
                                  </div>
                               </div>
                            ...
                            <div className="grid grid-cols-2 gap-8">
                               <div className="space-y-4">
                                  ...
                                  <div className="aspect-[16/10] bg-white/[0.02] rounded-[32px] overflow-hidden relative group/img cursor-pointer border border-white/5 hover:border-primary/40 transition-all duration-700 shadow-xl">
                                     {u.passport_image ? (
                                       <img src={u.passport_image} className="w-full h-full object-cover opacity-60 group-hover/img:scale-110 group-hover/img:opacity-100 transition-all duration-1000" alt="" />
                                     ) : (
                               ...
                               <div className="space-y-4">
                                  ...
                                  <div className="aspect-[16/10] bg-white/[0.02] rounded-[32px] overflow-hidden relative group/img cursor-pointer border border-white/5 hover:border-blue-500/40 transition-all duration-700 shadow-xl">
                                     {u.driver_license_image ? (
                                       <img src={u.driver_license_image} className="w-full h-full object-cover opacity-60 group-hover/img:scale-110 group-hover/img:opacity-100 transition-all duration-1000" alt="" />
                                     ) : (
                                       <div className="w-full h-full flex flex-col items-center justify-center italic text-[10px] text-white/10 uppercase gap-3">
                                          <AlertCircle className="w-6 h-6" /> Rasm mavjud emas
                                       </div>
                                ...
                                <div className="flex flex-col md:flex-row justify-between items-start gap-6">
                                   <div>
                                    ...
                                    <div className="flex flex-col items-end gap-3">
                                       <div className={`px-5 py-2 rounded-full text-[9px] font-bold uppercase tracking-[0.2em] shadow-lg ${
                                          b.status === 'approved' ? 'bg-green-500/10 text-green-500 border border-green-500/20 shadow-green-500/10' :
