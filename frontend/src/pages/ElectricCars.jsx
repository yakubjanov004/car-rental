...
const BENEFIT_ICONS = [DollarSign, Sparkles, Leaf];
const BENEFIT_ACCENTS = ['#00D97E', '#3B82F6', '#10B981'];
...
const ElectricCars = () => {
  ...
  const benefits = t('evPage.benefits', { returnObjects: true }) || [];
  ...
  useEffect(() => {
    const loadEVs = async () => {
      ...
      try {
        ...
        setMashinalar(evs.length > 0 ? evs : []);
      } catch (err) {
  ...
  return (
    <div className="bg-[#0A0A0A] min-h-screen pt-32 pb-40 overflow-hidden">
      ...
      <section className="relative h-[85vh] min-h-[700px] flex items-center overflow-hidden">
        ...
        >
          <img src={evHeroImg} alt="EV Background" className="w-full h-full object-cover" />
          ...
          <div className="max-w-3xl">
            <ScrollReveal direction="up">
              ...
              <h1 className="font-display text-7xl md:text-[8rem] font-extrabold tracking-tighter mb-10 leading-[0.85] text-white">
                {t('evPage.title1')} <span className="text-[#00D97E] italic">{t('evPage.title2')}</span><br />
                {t('evPage.title3')}
              </h1>
              <p className="max-w-xl text-white/60 text-lg md:text-xl font-light leading-relaxed mb-16 italic">
                "{t('evPage.description')}"
              </p>
            </ScrollReveal>
          </div>
        </div>
      ...
      <section className="py-32 px-6 border-y border-white/[0.04] bg-white/[0.01] relative">
        ...
        <div className="max-w-7xl mx-auto relative z-10">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {benefits.map((item, i) => {
              const IconComp = BENEFIT_ICONS[i] || DollarSign;
              const accent = BENEFIT_ACCENTS[i] || '#00D97E';
              return (
                <ScrollReveal key={i} direction="up" delay={i * 0.1}>
                  <div className="glass p-10 group hover:border-[#00D97E]/40 border-white/5 transition-all duration-500 rounded-[40px]">
                    <div className="w-16 h-16 rounded-2xl flex items-center justify-center mb-8 transition-all duration-700 bg-white/5 group-hover:bg-[#00D97E]/10 group-hover:scale-110">
                      <IconComp className="w-7 h-7" style={{ color: accent }} />
                    </div>
            ...
            <div className="glass p-12 md:p-24 relative overflow-hidden rounded-[64px] border-white/5 border-2">
               ...
               <div className="grid grid-cols-1 lg:grid-cols-2 gap-20 items-center relative z-10">
                  <div className="space-y-10">
                    <div className="text-[10px] text-[#00D97E] font-black uppercase tracking-[0.3em] mb-4">{t('evPage.compBadge')}</div>
                    <h2 className="font-display text-5xl md:text-7xl font-extrabold tracking-tighter leading-none">
                      {t('evPage.compTitle1')} <br /> <span className="text-white/30 italic">{t('evPage.compTitle2')}</span>
                    </h2>
                    ...
                    <div className="space-y-4">
                      <div className="flex justify-between items-center p-6 rounded-2xl bg-white/5 border border-white/5 group hover:bg-white/10 transition-colors">
                        <span className="text-white/30 text-xs font-bold uppercase tracking-widest">{t('evPage.gasLabel')}</span>
                      ...
                      <div className="flex justify-between items-center p-6 rounded-2xl bg-[#00D97E]/10 border border-[#00D97E]/20 group hover:bg-[#00D97E]/15 transition-colors">
                        <span className="text-[#00D97E] text-xs font-bold uppercase tracking-widest">{t('evPage.evLabel')}</span>
                  ...
                  <div className="glass bg-[#0F0F0F] p-16 rounded-[48px] border-white/5 flex flex-col items-center justify-center text-center shadow-2xl relative">
                    ...
                    <p className="text-[9px] text-white/30 uppercase font-black tracking-[0.4em] mb-4">{t('evPage.profitLabel')}</p>
                    <div className="font-display text-7xl md:text-8xl font-extrabold text-[#00D97E] leading-none mb-4 tracking-tighter">102K</div>
                    <p className="text-white/20 text-xs font-bold uppercase tracking-widest">{t('evPage.profitPer')}</p>
                  </div>
               </div>
            </div>
          </ScrollReveal>
      ...
      <section className="py-32 px-6 max-w-7xl mx-auto">
        <ScrollReveal direction="up" className="mb-20">
          <div className="flex flex-col md:flex-row items-end justify-between gap-8">
            <div>
              <div className="text-[10px] text-[#00D97E] font-black uppercase tracking-[0.3em] mb-6">{t('evPage.fleetBadge')}</div>
              <h2 className="font-display text-5xl md:text-7xl font-extrabold tracking-tighter leading-none">
                {t('evPage.fleetTitle1')} <span className="text-white/30 italic">{t('evPage.fleetTitle2')}</span>
              </h2>
            </div>
           ...
           <>
            {mashinalar.length > 0 ? (
               ...
               <div className="py-24 text-center glass rounded-[48px] border-white/5">
                  ...
                  <h3 className="text-2xl font-bold mb-2">{t('evPage.emptyTitle')}</h3>
                  <p className="text-white/30 text-sm">{t('evPage.emptyDesc')}</p>
               </div>
            )}
           </>
        )}
